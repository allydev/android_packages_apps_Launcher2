...
void fling() {
    g_LastTouchDown = 0;
    g_PosVelocity = -state->flingVelocity * ROWS_PER_PAGE;
...
void updatePos() {
    ...
    if (fabsf(g_PosVelocity) < COLUMNS_PER_PAGE || (g_PosVelocity * accel) < 0) {
        g_PosVelocity += accel;
    }
...
int positionStrip(float row, float column, int isTop, float p, int isText)
{
    ...
    float scale = 72.f * (COLUMNS_PER_PAGE - 1) / getWidth();
...
void drawFrontGrid(float rowOffset, float p)
{
    ...
    float yoff = h - ((h - (rowHeight * ROWS_PER_PAGE)) / 2);


    int row, col, iconNum;
    float ymin,ymax,gridTop,gridBottom;
    yoff -= 90;
    iconNum = intRowOffset * COLUMNS_PER_PAGE;
    ymax = yoff + rowHeight;
    gridTop = yoff;
    if(w > h )
        ymin = yoff - ((ROWS_PER_PAGE - 1) * rowHeight) - 70;
    else
        ymin = yoff - ((ROWS_PER_PAGE - 1) * rowHeight) - 120;
    gridBottom = ymin;
    gridBottom +=  110;
    ...
    for (row = 0; row < 5; row++) {
        ...
        for (col=0; col < COLUMNS_PER_PAGE; col++) {
            if (iconNum >= state->iconCount) {
                return;
            }

            if (iconNum >= 0) {
                float x = colWidth * col - ((ICON_TEXTURE_WIDTH_PX - colWidth) / 2);
                if ((y >= ymin) && (y <= ymax)) {
                    float iconY = y - 20;
                    setColor(1.f, 1.f, 1.f, 1.f);
                    if (state->selectedIconIndex == iconNum && !p) {
                        bindTexture(NAMED_PFTexNearest, 0, state->selectedIconTexture);
                        drawSpriteScreenspace(x, iconY, 0, ICON_TEXTURE_WIDTH_PX, ICON_TEXTURE_WIDTH_PX);
                    }
                    ...
                    if (!p) {
                        ...
                        drawSpriteScreenspaceCropped(x, iconY+cropB, 0, ICON_TEXTURE_WIDTH_PX, ICON_TEXTURE_WIDTH_PX-cropT-cropB,
                                0, ICON_TEXTURE_WIDTH_PX-cropB, ICON_TEXTURE_WIDTH_PX, -ICON_TEXTURE_WIDTH_PX+cropT+cropB);
                    } else {
...
void drawBottom(float rowOffset, float p)
{
    ...
    int row, col;
    int iconNum = (intRowOffset + (ROWS_PER_PAGE - 1)) * COLUMNS_PER_PAGE;
    while (1) {
        for (col=0; col < COLUMNS_PER_PAGE; col++) {
            if (iconNum >= state->iconCount) {
                return;
...
main(int launchID)
{
    ...
    g_PosMax = ((iconCount + (COLUMNS_PER_PAGE - 1)) / COLUMNS_PER_PAGE) - ROWS_PER_PAGE;
