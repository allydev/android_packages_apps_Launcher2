...
public class AllAppsView extends RSSurfaceView
    ...
    static class Defines {
        ...
        public static int ICON_WIDTH_PX = 64;
        public static int ICON_TEXTURE_WIDTH_PX = 128;

        public static int ICON_HEIGHT_PX = 64;
        public static int ICON_TEXTURE_HEIGHT_PX = 128;

        public int SCREEN_WIDTH_PX;
        public int SCREEN_HEIGHT_PX;
        public float SCREEN_DENSITY;

        public void recompute(int w, int h, float density) {
            SCREEN_WIDTH_PX = w;
            SCREEN_HEIGHT_PX = h;
    ...
    public class RolloRS {
        ...
        public void init(Resources res, int width, int height) {
            ...
            mWidth = width;
            mHeight = height;
            mDefines.recompute(width, height, res.getDisplayMetrics().density);
            initProgramVertex();
            initProgramFragment();
            initProgramStore();
            initMesh();
        ...
        void initTouchState() {
            ...
            mTouchYBorders[0] = centerY - ((cellHeight * Defines.ROWS_PER_PAGE) / 2);
            for (int i = 1; i <= Defines.ROWS_PER_PAGE; i++) {
                mTouchYBorders[i] =  mTouchYBorders[i - 1] + cellHeight;
	    }

            int centerX = (width / 2);
            mTouchXBorders[0] = centerX - ((cellWidth * Defines.COLUMNS_PER_PAGE) / 2);
            for (int i = 1; i <= Defines.COLUMNS_PER_PAGE; i++) {
                mTouchXBorders[i] = mTouchXBorders[i - 1] + cellWidth;
	    }
        }
        ...
        int chooseTappedIcon(int x, int y, float pos) {
            ...
            int index = (((int)pos) * Defines.COLUMNS_PER_PAGE)
                    + (row * Defines.COLUMNS_PER_PAGE) + col;
